...
sys.path.insert(0,module_dir);sys.path.insert(0,fixture_dir)
import tuned_runtime_probe as probe
import fake_tuned_resolver as fake
probe.install(fixture_dir)
if mode=='gil':
    errors=[];answers=[]
    def owner():
        try: answers.append(probe.resolve('slow_kernel','/test/a.py',False,0))
        except Exception as e: errors.append(repr(e))
    t=threading.Thread(target=owner,daemon=True);t.start()
    assert fake.entered.wait(5)
    answers.append(probe.resolve('slow_kernel','/test/a.py',False,0))
    t.join(5)
    assert not t.is_alive() and not errors and len(answers)==2 and answers[0]['calls']==1 and answers[1]['calls']==1
    print(json.dumps({'case':'real_gil_wait','pass':True}));raise SystemExit(0)
if mode=='identity':
    ...
    print(json.dumps({'case':'builtin_source_identity','pass':ok,'a':a,'b':b}));raise SystemExit(0 if ok else 1)
...
original=compile_module.compile_a_kernel;calls=[]
...
stream.synchronize();assert torch.equal(y,x*2)
try:
    probe.launch(path,x.data_ptr(),y.data_ptr(),x.numel(),256,stream.cuda_stream,True)
...
with torch.cuda.graph(graph,stream=stream):
    probe.resolve('mm_kernel','/test/a.py',False,stream.cuda_stream)
    probe.launch(path,x.data_ptr(),y.data_ptr(),x.numel(),128,stream.cuda_stream,False)
for i in range(5):
    x.fill_(i+1);graph.replay();torch.cuda.synchronize()
...
for kind in ['config','program']:
    ...
    with torch.cuda.graph(empty,stream=stream):
        try:
            if kind=='config': probe.resolve('capture_cold','/test/missing.py',False,stream.cuda_stream)
            else: probe.launch(path,x.data_ptr(),y.data_ptr(),x.numel(),512,stream.cuda_stream,False)
        except RuntimeError as error:
            denied='captur' in str(error)
    assert denied
assert len(calls)==1 and fake._calls==before
print(json.dumps({'case':'prepare_freeze_capture','pass':True,'compiler_calls':len(calls),'replays':5}))
