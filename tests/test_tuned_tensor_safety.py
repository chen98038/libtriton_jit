...
spec = importlib.util.spec_from_file_location('safety_fixture', Path(__file__).with_name('test_tuned_resolver.py'))
...
def kernel(x, M, stride0, stride1, BLOCK): pass

def make(names, keys):
    return fixture.LibTuner('safety', fixture.JITFunction(kernel, names), keys, None,
                           [fixture.Config({'BLOCK':16}),fixture.Config({'BLOCK':32})], [1.,2.]), fixture.fake_libentry('flag_fake')
...
check('slice_layout', b[0].stride()==x.stride() and torch.equal(b[0],x), original=list(x.stride()), scratch=list(b[0].stride()))
check('slice_bounds', sum((n-1)*s for n,s in zip(b[0].shape,b[2:]))+b[0].storage_offset() < b[0].untyped_storage().nbytes()//b[0].element_size())
...
x=torch.ones(4)
def reset(self,named,reset_only=False): named['x'].zero_()
...
a=root[1:5,::2]; b=root.T
args,kw=resolver._clone_arguments((a,),{'b':b})
check('offset_alias', args[0].storage_offset()==a.storage_offset() and args[0].stride()==a.stride()
      and kw['b'].stride()==b.stride() and args[0].untyped_storage()._cdata==kw['b'].untyped_storage()._cdata
...
entered=threading.Event(); second_done=threading.Event(); errors=[]
def interleave(self,M,config,**kwargs):
    if M==7:
        entered.set()
        if not second_done.wait(3): raise TimeoutError('second call did not finish')
    if self.nargs['M']!=M: raise AssertionError('another invocation replaced nargs')
    return (1.,1.,1.)
tuner._bench=types.MethodType(interleave,tuner)
def first():
    try: resolver.resolve_with_tuner(tuner,lib,(7,),{},grid=(1,))
    except Exception as error: errors.append(repr(error))
worker=threading.Thread(target=first,daemon=True);worker.start()
if not entered.wait(3): raise TimeoutError('first call did not enter')
resolver.resolve_with_tuner(tuner,lib,(9,),{},grid=(1,))
second_done.set();worker.join(4)
